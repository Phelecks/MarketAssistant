...
public record GetRpcUrlsQuery(bool bypassCache, string cacheKey, int? expireInMinutes = 24 * 60) : PagingInformationRequest, IRequest<PaginatedList<RpcUrlDto>>, ICacheableAddDomainNotificationQuery;
...
public class Handler(IApplicationDbContext context, IMapper mapper) : IRequestHandler<GetRpcUrlsQuery, PaginatedList<RpcUrlDto>>
{
    ...
    public async Task<PaginatedList<RpcUrlDto>> HandleAsync(GetRpcUrlsQuery request, CancellationToken cancellationToken)
    {
